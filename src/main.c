...
void	my_keyhook(mlx_key_data_t keydata, t_data *data)
{
	...
	if (keydata.key == MLX_KEY_UP && keydata.action == MLX_PRESS)
		data->scene->c.center = vec_sub(camera.center, vec_mult(camera.up, 0.01));
	if (keydata.key == MLX_KEY_DOWN && keydata.action == MLX_PRESS)
		data->scene->c.center = vec_add(camera.center, vec_mult(camera.up, 0.01));
	if (keydata.key == MLX_KEY_RIGHT && keydata.action == MLX_PRESS)
		data->scene->c.center = vec_sub(camera.center, vec_mult(camera.right, 0.01));
	if (keydata.key == MLX_KEY_LEFT && keydata.action == MLX_PRESS)
		data->scene->c.center = vec_add(camera.center, vec_mult(camera.right, 0.01));
