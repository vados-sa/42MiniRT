...
/*   Updated: 2024/12/18 12:59:31 by pbencze          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color	specular_light(t_l *light, t_intersec *hit_rec, t_data *data)
{
	...
	specular.shine = 5.0;
	specular.strength = 0.9;
	result = col(0, 0, 0, 255);
	specular.view_dir = \
		vec_unit(vec_sub(data->scene->c.center, hit_rec->point));
	while (light)
	{
		specular.light_dir = vec_unit(vec_sub(light->point, hit_rec->point));
		specular.reflect_dir = \
			vec_reflect(vec_mult(specular.light_dir, -1.0), hit_rec->normal);
		specular.spec = \
			pow(fmax(vec_dot(specular.view_dir, specular.reflect_dir), 0.0), \
			specular.shine);
		specular.temp_specular = \
			col_mult(light->color, specular.spec * specular.strength);
...
t_color	calculate_light(t_data *data, t_intersec *hit_rec, t_color color)
{
	...
	specular = col(0, 0, 0, 0);
	if (SPECULAR == 1)
		specular = specular_light(data->scene->l, hit_rec, data);
	result.r = \
	((t_float)(ambient.r + diffuse.r + specular.r) / 255.0) * color.r;
	result.g = \
	((t_float)(ambient.g + diffuse.g + specular.g) / 255.0) * color.g;
	result.b = \
	((t_float)(ambient.b + diffuse.b + specular.b) / 255.0) * color.b;
	if (result.r > 255.0)
		result.r = 255.0;
	if (result.g > 255.0)
		result.g = 255.0;
